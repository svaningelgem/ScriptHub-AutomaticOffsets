...
IC_CORE_ROOT = Path(__file__).parent.parent.joinpath("Addons/IC_Core/MemoryRead")
...
def StartImport(memFileLoc, is64Bit):
    ...
    if not memFileLoc.exists():
        ...
        return
    Path(IC_CORE_ROOT, "Imports", "ActiveEffectHandlers").mkdir(parents=True, exist_ok=True)
...
def OutputImportToFile(fileNameBase, version, isEffectHandler):
    ...
    outputFile = Path(IC_CORE_ROOT, "Imports", extraFolder, "IC_" + fileNameBase + version + "_Import.ahk")
...
def OutputHandlerIncludeFile(count, is64Bit ):
    if count > 0:
        ...
        handlerImportsFile = Path(IC_CORE_ROOT, "Imports", "ActiveEffectHandlers", "IC_HeroHandlerIncludes" + version + "_Import.ahk")
...
def CreateVersionFile(architecture):
    ...
    outputFile = Path(IC_CORE_ROOT, "Imports", "IC_GameVersion" + str(architecture) + "_Import.ahk")
